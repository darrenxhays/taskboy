"""live credential validators for the setup wizard and `taskboy setup --check`.
...
def check_aws_role(role_arn: str, external_id: str) -> tuple[bool, str]:
    try:
        ...
        sts.assume_role(RoleArn=role_arn, RoleSessionName="taskboy-setup-check", ExternalId=external_id, DurationSeconds=900)
    except Exception as e:
        return False, f"{type(e).__name__}: {e}"
