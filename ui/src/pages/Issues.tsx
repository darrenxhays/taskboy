...
import type { Issue, IssueAttachment, IssueComment } from "../api";
...
export function Issues({ admin, email }: { admin: boolean; email: string | null }) {
  ...
  // the detail panel is page-level and URL-keyed (?issue=<id>) rather than per-table local state, so a link
  // into it survives a refresh and isn't hidden by whichever table's repo filter or page the row falls on.
  const [searchParams, setSearchParams] = useSearchParams();
  const issueParam = searchParams.get("issue");
  const parsedIssueId = issueParam ? Number(issueParam) : NaN;
  const openIssueId = Number.isInteger(parsedIssueId) ? parsedIssueId : null;
  const openIssue = (id: number) => setSearchParams((prev) => { const next = new URLSearchParams(prev); next.set("issue", String(id)); return next; });
  const closeIssue = () => setSearchParams((prev) => { const next = new URLSearchParams(prev); next.delete("issue"); return next; }, { replace: true });
  const toggleIssue = (id: number) => (openIssueId === id ? closeIssue() : openIssue(id));
  ...
  const implement = (id: number) => mutate(() => api.implementIssue(id), `implementation started for issue #${id}`);
  const implementDisabled = busy || data?.implementation_active != null;
  const eligible = (action: "approve" | "deny" | "refine" | "delete" | "implement") => [...selected].filter((id) => { const row = data?.issues.find((item) => item.id === id); if (action === "refine") return row != null && canRefine(row.status); if (action === "delete") return row != null && canDelete(row.status); if (action === "implement") return row != null && canImplement(row.status); return true; });
...
type TableProps = { title: string; rows: Issue[]; loaded: boolean; empty: string; tableKey: string; showRank: boolean; admin: boolean; busy: boolean; selected: Set<number>; setSelected: (selected: Set<number>) => void; decide: (id: number, decision: "approved" | "denied") => Promise<unknown>; refine: (id: number) => Promise<unknown>; remove: (id: number) => Promise<unknown>; implement: (id: number) => Promise<unknown>; implementDisabled: boolean; openIssueId: number | null; onToggle: (id: number) => void };

function IssueTable({ title, rows, loaded, empty, tableKey, showRank, admin, busy, selected, setSelected, decide, refine, remove, implement, implementDisabled, openIssueId, onToggle }: TableProps) {
...
function DetailPane({ open, issueId, admin, email, busy, decide, refine, remove, implement, implementDisabled, reload, setNote, onClose }: { open: boolean; issueId: number | null; admin: boolean; email: string | null; busy: boolean; decide: (id: number, decision: "approved" | "denied") => Promise<unknown>; refine: (id: number) => Promise<unknown>; remove: (id: number) => Promise<unknown>; implement: (id: number) => Promise<unknown>; implementDisabled: boolean; reload: () => void; setNote: (note: string | null) => void; onClose: () => void }) {
  // retain the last non-null id while closing, so the pane doesn't go blank and the heading doesn't flip mid-transition
  const [displayId, setDisplayId] = useState(issueId);
  useEffect(() => { if (issueId != null) setDisplayId(issueId); }, [issueId]);
  return (
    <>
      ...
      >
        <div className="mb-3 flex items-center justify-between gap-2">
          <h2 className="panel-title truncate">{displayId != null ? `Issue #${displayId}` : "Issue"}</h2>
...
function IssueCard({ row, admin, busy, expanded, selected, onToggle, onSelect, decide, refine, remove, implement, implementDisabled, showRank }: { row: Issue; admin: boolean; busy: boolean; expanded: boolean; selected: boolean; onToggle: () => void; onSelect: () => void; decide: (id: number, decision: "approved" | "denied") => Promise<unknown>; refine: (id: number) => Promise<unknown>; remove: (id: number) => Promise<unknown>; implement: (id: number) => Promise<unknown>; implementDisabled: boolean; showRank: boolean }) {
  ...
  return (
    <RowCard onClick={onToggle} selected={expanded}>
      ...
      {admin && <CardActions className="justify-center">
        ...
        {canImplement(row.status) && <ConfirmIconButton onConfirm={() => implement(row.id)} disabled={implementDisabled} title="implement now" borderColor="var(--status-good)" color="var(--status-good)"><RocketIcon /></ConfirmIconButton>}
        {canDelete(row.status) && <ConfirmIconButton onConfirm={() => remove(row.id)} disabled={busy} title="delete" borderColor="var(--hairline-strong)" color="var(--text-muted)"><TrashIcon /></ConfirmIconButton>}
      </CardActions>}
    </RowCard>
  );
}

function ExpandedIssue({ issueId, admin, email, busy, decide, refine, remove, implement, implementDisabled, reload: reloadList, setNote, onDeleted }: { issueId: number; admin: boolean; email: string | null; busy: boolean; decide: (id: number, decision: "approved" | "denied") => Promise<unknown>; refine: (id: number) => Promise<unknown>; remove: (id: number) => Promise<unknown>; implement: (id: number) => Promise<unknown>; implementDisabled: boolean; reload: () => void; setNote: (note: string | null) => void; onDeleted: () => void }) {
  const { data: detail, error: detailError, reload: reloadDetail } = useLiveData(() => api.issue(issueId), [issueId]);
  const [editing, setEditing] = useState(false);
  const [summary, setSummary] = useState("");
  const [description, setDescription] = useState("");
  // null = no unsaved slider edit yet, so live refreshes (e.g. a refine task's new priority) keep showing
  ...
  const savePriority = async () => { try { await api.setIssuePriority(issueId, displayPriority); await reloadDetail(); setPriority(null); reloadList(); } catch (reason) { setPriority(null); setNote((reason as Error).message); } };
  const changedPriority = priority !== null && priority !== issue.priority;
  const refresh = async () => { await reloadDetail(); reloadList(); };
  ...
  return <div className="space-y-4 rounded-md border p-3" style={{ borderColor: "var(--hairline)", background: "var(--surface-2)" }}><div>{editing ? <div className="space-y-2"><input value={summary} onChange={(event) => setSummary(event.target.value)} className={`${fieldClass} w-full`} style={fieldStyle} /><MarkdownEditor value={description} onChange={setDescription} /><div className="flex gap-2"><button onClick={saveDetails} disabled={!summary.trim() || !description.trim()} className="rounded border px-3 py-1 text-[11px]" style={{ borderColor: "var(--status-good)", color: "var(--status-good)" }}>✓ save</button><button onClick={() => setEditing(false)} className="rounded border px-3 py-1 text-[11px]" style={{ borderColor: "var(--status-critical)", color: "var(--status-serious)" }}>✗ undo</button></div></div> : <div><div className="flex flex-wrap items-start justify-between gap-2"><h3 className="text-[15px] font-semibold">{issue.summary}</h3>{editable && <button onClick={startEditing} className="text-[11px]" style={{ color: "var(--accent)" }}>edit</button>}</div><Markdown text={issue.details} /></div>}</div>
    ...
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="flex flex-wrap items-center gap-2"><label className="text-[11px] font-semibold">Priority {displayPriority}</label><input type="range" min={1} max={100} value={displayPriority} disabled={!editable} onChange={(event) => setPriority(Number(event.target.value))} className="w-24" />{changedPriority && <><button onClick={savePriority} className="rounded border px-2 py-0.5 text-[11px]" style={{ borderColor: "var(--status-good)", color: "var(--status-good)" }}>✓</button><button onClick={() => setPriority(null)} className="rounded border px-2 py-0.5 text-[11px]" style={{ borderColor: "var(--status-critical)", color: "var(--status-serious)" }}>✗</button></>}</div>
      {admin && <div className="flex items-center gap-1.5">
        {!decisionLocked && <><IconButton onClick={async () => { await decide(issueId, "approved"); reloadDetail(); }} disabled={busy} title="approve" borderColor="var(--status-good)" color="var(--status-good)" filled={issue.status === "approved"}><CheckIcon /></IconButton><IconButton onClick={async () => { await decide(issueId, "denied"); reloadDetail(); }} disabled={busy} title="deny" borderColor="var(--status-critical)" color="var(--status-serious)" filled={issue.status === "denied"}><XIcon /></IconButton></>}
        {canRefine(issue.status) && <IconButton onClick={async () => { await refine(issueId); reloadDetail(); }} disabled={busy || issue.refine_task_id != null} title={issue.refine_task_id ? "refining" : "refine"} borderColor="var(--accent)" color="var(--accent)"><RefreshIcon spinning={issue.refine_task_id != null} /></IconButton>}
        {canImplement(issue.status) && <ConfirmIconButton onConfirm={() => implement(issueId)} disabled={implementDisabled} title="implement now" borderColor="var(--status-good)" color="var(--status-good)"><RocketIcon /></ConfirmIconButton>}
