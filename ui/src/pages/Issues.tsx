...
export function Issues({ admin, email }: { admin: boolean; email: string | null }) {
  ...
  const eligible = (action: "approve" | "deny" | "refine" | "delete") => [...selected].filter((id) => { const row = data?.issues.find((item) => item.id === id); if (action === "refine") return row != null && canRefine(row.status); if (action === "delete") return row != null && canDelete(row.status); return true; });
  const bulk = async (action: "approve" | "deny" | "refine" | "delete") => { const ids = eligible(action); if (!ids.length) return; try { await mutate(async () => { const result = await api.bulkIssues(ids, action); const successes = result.results.filter((row) => !["skipped", "not_found", "already_running"].includes(row.status)).length; setNote(`${action}: ${successes} succeeded, ${result.results.length - successes} skipped`); setSelected(new Set()); }); } catch { /* note is set by mutate */ } };
  ...
  return <div className="mx-auto max-w-7xl space-y-4 overflow-x-hidden"><header className="flex flex-wrap items-center justify-between gap-3"><h1 className="text-[18px] font-bold tracking-wide">ISSUES</h1><div className="flex flex-wrap items-center gap-2"><RepoFilter repos={repos} selected={effectiveRepos} onChange={setSelectedRepos} />{admin && <><button onClick={() => setCreating(true)} disabled={busy || creating} className="rounded-md px-3 py-1.5 text-[12px] font-bold text-white disabled:opacity-40" style={{ background: "var(--accent)" }}>CREATE</button><div className="flex flex-wrap items-center gap-1"><select value={discoveryRepo} onChange={(event) => setDiscoveryRepo(event.target.value)} className="max-w-56 rounded-md border px-2 py-1.5 text-[12px]" style={fieldStyle}>{repos.map((repo) => <option key={repo}>{repo}</option>)}</select><button onClick={() => run("discoverissues")} disabled={busy || !discoveryRepo} className="rounded-md border px-3 py-1.5 text-[12px] font-bold disabled:opacity-40" style={{ borderColor: "var(--accent)", color: "var(--accent)" }}>RUN DISCOVERY</button></div><button onClick={() => run("implementapprovedissues")} disabled={busy || data?.implementation_active != null} className="rounded-md border px-3 py-1.5 text-[12px] font-bold disabled:opacity-40" style={{ borderColor: "var(--status-good)", color: "var(--status-good)" }}>{data?.implementation_active ? "IMPLEMENTING" : "IMPLEMENT APPROVED"}</button></>}</div></header>
    ...
    {admin && selected.size > 0 && <div className="sticky top-0 z-[5] flex flex-wrap items-center gap-2 rounded-md border p-3" style={{ borderColor: "var(--accent)", background: "var(--surface-1)" }}><span className="text-[12px] font-semibold">{selected.size} selected</span>{(["approve", "deny", "refine"] as const).map((action) => <IconButton key={action} onClick={() => bulk(action)} disabled={busy} title={action} borderColor={action === "approve" ? "var(--status-good)" : action === "deny" ? "var(--status-critical)" : "var(--accent)"} color={action === "approve" ? "var(--status-good)" : action === "deny" ? "var(--status-serious)" : "var(--accent)"}>{action === "approve" ? <CheckIcon /> : action === "deny" ? <XIcon /> : <RefreshIcon />}</IconButton>)}{deletable.length > 0 && <ConfirmIconButton onConfirm={() => bulk("delete")} disabled={busy} title="delete" label={`Delete ${deletable.length} issue${deletable.length === 1 ? "" : "s"}?`} borderColor="var(--status-critical)" color="var(--status-serious)"><TrashIcon /></ConfirmIconButton>}<button onClick={() => setSelected(new Set())} className="text-[11px]" style={{ color: "var(--text-muted)" }}>clear</button></div>}
    <IssueTable title="In progress" rows={rows.filter((row) => row.status === "in_review" || row.status === "in_progress")} loaded={data != null} empty="nothing in progress" tableKey="in-review" showRank={false} {...{ admin, busy, selected, setSelected, decide, refine, remove, openIssueId, onToggle: toggleIssue }} />
    <IssueTable title="Active" rows={rows.filter((row) => ACTIVE_STATUSES.includes(row.status))} loaded={data != null} empty={effectiveRepos.length ? "no active issues" : "select at least one repo"} tableKey="active" showRank {...{ admin, busy, selected, setSelected, decide, refine, remove, openIssueId, onToggle: toggleIssue }} />
    <IssueTable title="Done / Denied" rows={rows.filter((row) => ARCHIVE_STATUSES.includes(row.status))} loaded={data != null} empty="nothing finished or denied" tableKey="archive" showRank={false} {...{ admin, busy, selected, setSelected, decide, refine, remove, openIssueId, onToggle: toggleIssue }} />
    <DetailPane open={openIssueId != null} issueId={openIssueId} admin={admin} email={email} busy={busy} decide={decide} refine={refine} remove={remove} reload={reload} setNote={setNote} onClose={closeIssue} />
  </div>;
}

type TableProps = { title: string; rows: Issue[]; loaded: boolean; empty: string; tableKey: string; showRank: boolean; admin: boolean; busy: boolean; selected: Set<number>; setSelected: (selected: Set<number>) => void; decide: (id: number, decision: "approved" | "denied") => Promise<unknown>; refine: (id: number) => Promise<unknown>; remove: (id: number) => Promise<unknown>; openIssueId: number | null; onToggle: (id: number) => void };

function IssueTable({ title, rows, loaded, empty, tableKey, showRank, admin, busy, selected, setSelected, decide, refine, remove, openIssueId, onToggle }: TableProps) {
...
function ExpandedIssue({ issueId, admin, email, busy, decide, refine, remove, reload: reloadList, setNote, onDeleted }: { issueId: number; admin: boolean; email: string | null; busy: boolean; decide: (id: number, decision: "approved" | "denied") => Promise<unknown>; refine: (id: number) => Promise<unknown>; remove: (id: number) => Promise<unknown>; reload: () => void; setNote: (note: string | null) => void; onDeleted: () => void }) {
  ...
  const [priority, setPriority] = useState(50);
  const load = async () => { setLoading(true); try { const value = await api.issue(issueId); setDetail(value); setSummary(value.issue.summary); setDescription(value.issue.details); setPriority(value.issue.priority); } catch (reason) { setDetail(null); setNote((reason as Error).message); } finally { setLoading(false); } };
  ...
  if (!detail) return <ErrorNote message="this issue could not be loaded" />;
