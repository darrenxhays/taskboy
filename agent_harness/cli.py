...
from agent_harness.models import CANCELLED, TERMINAL_STATES, utcnow
...
def main() -> None:
    ...
    from agent_harness import setup_wizard

    setup_wizard.add_parser(sub)

    args = parser.parse_args()
    if args.command == "run":
        from agent_harness.main import main as run_main
...
def _decide_permission(store: Store, decision_verb: str, task_id: str, kind: str, target: str) -> None:
    from agent_harness.task_actions import decide_permission
...
def _cancel(store: Store, task_id: str) -> None:
    ...
    try:
        ...
        print(f"{task_id} changed state while cancelling — check `agent-harness list` and retry")
