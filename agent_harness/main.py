...
from agent_harness import repocache, settings, workspace
from agent_harness.classifier import Classifier, stub_classify
from agent_harness.config import ConfigError, load_config, role_for
from agent_harness.notify import StdoutNotifier
from agent_harness.orchestrator import Orchestrator
from agent_harness.runner import ClaudeRunner, EchoRunner
...
async def amain() -> None:
    ...
    if config.slack.enabled or config.runner == "claude" or config.dashboard.enabled:
        from agent_harness.secrets import load_secrets
    ...
    if config.slack.enabled:
        ...
        from agent_harness import slack
        ...
        logger.info("slack intake disabled (slack.team_id not set) — use `agent-harness inject`")

    broker = None
    reviewer_broker = None
    if config.runner == "claude":
        from agent_harness.broker import CredentialBroker
    ...
    if should_start_review_poller(config, broker):
        from agent_harness.review_requests import ReviewRequestPoller
    ...
    async def housekeeping_loop() -> None:
        ...
        from agent_harness.issue_runs import fail_stalled_implementation_run, sync_in_review
    ...
    scheduler_task = None
    if config.runner == "claude":
        from agent_harness.scheduler import scheduler_loop, seed_default_schedules
    ...
    logger.info("agent-harness started (environment=%s, db=%s, audit_shipping=%s)", settings.ENVIRONMENT, settings.DB_PATH, bool(audit_bucket))
