...
never reaches the model. created issues always carry the `agent-harness` + `agent-task-{id}` labels and
...
logger = logging.getLogger("agent_harness.jira")
...
class JiraAdapter:
    ...
    async def create_issue(self, args: dict) -> dict:
        ...
        footer = f"Created by {self.bot_name} (agent-harness) for Slack task {self.task.task_id} in <#{self.task.slack_channel_id}>."
        fields: dict = {
            "project": {"key": project},
            "issuetype": {"name": issue_type},
            "summary": summary,
            "description": _adf(f"{description}\n\n{footer}" if description else footer),
            "labels": ["agent-harness", f"agent-task-{self.task.task_id}"],  # JIR-008
        }
