...
from agent_harness.models import utcnow
from agent_harness.orchestrator import accept_task
from agent_harness.store import Store

logger = logging.getLogger("agent_harness.scheduler")
...
CLI_UPDATE_PACKAGE = "claude-agent-sdk"  # bundles the claude cli (deploy/install.sh:24)
CLI_UPDATE_SERVICE_UNIT = "agent-harness"
# NoNewPrivileges=yes blocks sudo/setuid, so we can't restart the unit from inside this process. instead
# touch this flag; the root-owned agent-harness-restart.path unit watches it and restarts us (deploy/install.sh)
CLI_UPDATE_RESTART_FLAG = "/run/agent-harness/restart-requested"
...
async def run_cli_update(store: Store) -> None:
    """off-peak upgrade of the claude-agent-sdk package (which bundles the claude cli, deploy/install.sh:24)
    ...
    # sudo/systemctl can't work here (NoNewPrivileges=yes); the root-side agent-harness-restart.path unit watches
    # this flag and does the restart outside our sandbox (deploy/install.sh)
