...
from agent_harness import memory, repocache, settings, skills, workspace
from agent_harness.config import Config, role_for
from agent_harness.hooks import TOOL_CLASSIFICATION, TaskHooks, repo_grantable, tool_grantable
from agent_harness.models import BLOCKED, COMPLETED, FAILED, Outcome, Task
from agent_harness.personality import load as load_personality
from agent_harness.prompts import CONVENTIONS_FILENAME, task_prompt
from agent_harness.router import fallback_chain
...
class ClaudeRunner:
    ...
    async def _run_session(self, task: Task, model_id: str, ws, prompt: str, broker_env: dict[str, str], scoped_repos: list[str], task_broker=None, granted_tools: list[str] | None = None, skill_internal_tools: list[str] | None = None) -> Outcome:
        ...
        from agent_harness.adapters.issues import ENQUEUE_TOOLS, ISSUES_TOOLS, EnqueueAdapter, IssuesAdapter, build_enqueue_server, build_issues_server
        ...
        if task_broker is not None:
            from agent_harness.adapters.github_api import GitHubAdapter, build_github_server
        ...
        if self.secrets is not None and self.secrets.jira_enabled and jira_config.get("site"):
            from agent_harness.adapters.jira import JiraAdapter, build_jira_server
        ...
        if self.secrets is not None and self.secrets.jira_enabled and confluence_config.get("site"):
            from agent_harness.adapters.confluence import ConfluenceAdapter, build_confluence_server
        ...
        if self.secrets is not None and self.secrets.sentry_token and sentry_config.get("organization"):
            from agent_harness.adapters.sentry import SentryAdapter, build_sentry_server
        ...
        if aws_config.get("allowed_services"):
            from agent_harness.adapters.aws_read import AwsReadAdapter, build_aws_server
        ...
        if self.slack_client is not None:
            from agent_harness.adapters.slack_history import SlackHistoryAdapter, build_slack_server
