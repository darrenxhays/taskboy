...
Installed skills live here, one directory per skill: `skills/<name>/SKILL.md`. This directory ships **empty** — install skills from the packaged templates, either through the `taskboy setup` wizard (its skills picker copies the templates you choose and fills in every `{{variable}}`) or by hand: extract them with `taskboy assets templates /tmp/seed`, copy a template directory here, and replace every `{{variable}}` placeholder with your value (see the extracted `templates/skills/README.md` for the variable table).
